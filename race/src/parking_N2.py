	...
	def execute(self):
		...
		for circle in obstacles.circles:
			...
			if self.flag == 0 and self.mode == 0:
				if 0.3 < x < 0.454 and -1 < y < 0:			# 만약 lidar가 object를 (0.60 ~ 0.62, -1 ~ 0)(기둥)에서 감지하면
					print("2 :", "x:",x,"y:",y)	
				...
				if self.flag == 1: 
					...
					if (self.now_time - self.old_time > 4.2):	# operate to left steer after 2.5second
					...
					if 0.1 < x < 0.3:				# 주차선맞추기(오른쪽 wall과의 거리) -> 만약 lidar가 object를 (0.1 ~ -0.2, 0)에서 감지하면
					...
					if (self.old_time == 0): 
						self.old_time = time.time()
					if (self.now_time - self.old_time > 1.7): 	# 1.6초후 정지
						car_run_speed = 0
						self.mode = 0
					# else -> 앞으로 이동
					else:
